...
export const agentKeys = ['claude', 'codex', 'gemini', 'openclaw', 'apichat'] as const;
...
export const AgentDefaultOverridesSchema = z.object({
    ...
    apichat: AgentDefaultOverrideSchema.optional(),
}).passthrough().default({});
...
const codeAgentDefaults: Record<AgentKey, AgentDefaultConfig> = {
    ...
    apichat: { permissionMode: 'default', modelMode: 'default', effortLevel: null },
};

export function normalizeAgentKey(flavor: string | null | undefined): AgentKey {
    if (flavor === 'codex' || flavor === 'gemini' || flavor === 'openclaw' || flavor === 'apichat') {
        return flavor;
    }
    return 'claude';
}
